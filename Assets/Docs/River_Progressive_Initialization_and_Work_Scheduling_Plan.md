...
23. Patch 4.8B safe old/new generated-topology transition, superseding flattening, and dimension-changing domain/quality hold/remap — implemented; Unity validation pending;
...
18. Patch 4.8B safe generated-topology crossfade, superseding-transition flattening, and differently mapped domain/quality hold/remap — implemented; Unity validation pending.

Remaining topology slices:

1. Validate Patch 4.8B same-grid fade, rapid superseding activation, and differently mapped domain/quality transition;
2. Patch 4.9 production procedural chunk/run cache and precompute packaging;
...
Patch 4.7C.3.3 soft Connector distribution is visually/long-run validated. Patch 4.8A staged replacement preparation and atomic activation are implemented and Unity-verified. Patch 4.8B safe topology transition is implemented. Static and movement coefficient tuning remains provisional.

The immediate gate is Patch 4.8B Unity validation:

- change one same-grid topology setting after the active topology has evolved; confirm `Transition State` becomes `Crossfading`, progress advances continuously, and the fully resolved old generated result fades into the complete replacement without a black frame or additive support spike;
- inspect class diagnostics during the fade; Major, Connector, hosted-negative, Free-Water, and Weak Span old states must transition as one complete generated snapshot rather than exposing partial replacement phases;
- change another topology setting before the first fade completes; confirm `Flattened Transitions` increases and the current visible blend becomes the next transition source without synchronous completion;
- change live Pressure, Lee, Shore, or obstacle context during a fade; these authoritative sources must remain current and be composed only once outside the generated transition;
- change quality or river-domain mapping; confirm the previous complete renderer bindings remain visible while staged initialization progresses, then `Remapped Transitions` increases and the old generated field fades into the new mapping by global distance and lateral metres;
- inspect old/new non-overlap extents and verify old edge texels are not smeared into invalid new space;
- repeat transitions and monitor memory/diagnostics; captured textures, retained metric buffers, and held renderer resources must retire after completion rather than accumulate;
- confirm disabling, freezing, destroying, or superseding initialization during either hold or fade releases transition resources safely;
- confirm no normal-runtime readback, path search, retry loop, additive old/new composition, or per-region transition dispatch was introduced.

After Patch 4.8B validation, proceed to Patch 4.9 production cache/precompute packaging. This is the phase intended to remove the temporary startup generation freeze by loading accepted prepared topology and exact obstacle data rather than regenerating them on ordinary Play entry.
