...
22. Patch 4.8A staged same-grid replacement preparation and atomic activation — implemented; Unity validation pending;
23. Patch 4.8B bounded old/new transition, including dimension-changing domain/quality resource sets;
24. Patch 4.9 production procedural chunk/run cache and precompute packaging.
...
1. Validate Patch 4.8A same-grid replacement staging, identical preparation, superseding-request coalescing, and atomic activation;
...
Patch 4.7C.3.3 soft Connector distribution is visually/long-run validated. Patch 4.8A staged same-grid replacement preparation and atomic activation are implemented. Static and movement coefficient tuning remains provisional.

The immediate gate is Patch 4.8A Unity validation:

- while viewing topology diagnostics, change Major Seed, Major Amount/Size, Connector controls, and each negative Amount independently;
- confirm the currently accepted topology remains visible and continues ordinary evolution while `Replacement Build State` advances through Major, Connector, Pocket, upload, and readiness phases;
- confirm no partly rebuilt combination appears—for example new Majors with old Connectors or old Weak Spans;
- confirm the complete replacement activates only after readiness and there is no neutral/black topology frame;
- use **Prepare Identical Topology Replacement** after the active topology has evolved for a while; confirm `Identical Preparations` increases and the visible topology does not reset or change because the prepared result is discarded rather than activated;
- change one topology setting repeatedly before preparation completes; confirm `Coalesced / Cancelled` increases, preparation restarts for the newest target, and the active topology is never cleared;
- change obstacle context and confirm the authoritative Obstacle Footprint refreshes first, then the settled scalar field is captured by a replacement request;
- confirm ordinary Major, hosted-negative, Free-Water, Connector, and Weak Span evolution continues during replacement CPU preparation and pauses only for the final activation frame;
- confirm disabling, freezing, destroying, or forcing a dimension-changing domain/quality reinitialization releases any temporary replacement texture safely;
- confirm no new compute shader, runtime readback, path search, retry loop, or per-region dispatch was introduced.

Patch 4.8A intentionally does not crossfade old and new generated topology. A direct shape change at atomic activation is therefore expected. Patch 4.8B is next and will retain both complete generated sets for a bounded fade, keep Anchored Support authoritative, and cover differently mapped domain/quality transitions.
