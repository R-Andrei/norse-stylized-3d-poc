...
- [ ] Pass 1 - Baseline and contract split: isolate public structs, diagnostics contracts, constants, property IDs, and internal state records while preserving serialized field names.
- [ ] Pass 2 - Resource and compute split: move compute loading, kernel lookup, texture/buffer allocation, release, shared binding, and dispatch helpers into focused partial files.
- [ ] Pass 3 - Source-path split: move generated-source, static-source, continuous-source, impact, pressure, wake, and boundary responsibilities into separate partials.
- [ ] Pass 4 - Long-method reduction and cleanup: reduce the largest methods, preserve static diagnostics, update this log, and validate emitter/generated-source workflows.
