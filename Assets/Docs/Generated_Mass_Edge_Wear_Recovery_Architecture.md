...
### EW-C1A.3d validation de-duplication ownership

The one-click validator is a research orchestration owner, not a geometry owner. It now schedules current preview and Macro contracts, one exhaustive 33-case topology construction, one non-emitting 33-case artistic fingerprint projection from the topology records, the 33-case corner matrix, and a 12-case difficult artistic materialization sentinel set. The direct focused artistic matrix remains exhaustive. Comprehensive artistic scenario evidence consumes the 33 projected records and performs no additional rock generation.

Fingerprint projection preserves stable source-edge identities, finite scores, deterministic descending rank order, and the exact artistic-eligible count. Deep sentinel results must reproduce the matching fingerprint before the suite can pass. A global abort marks untouched stages `not-run`. The research hard stop is `90 s`; no change is made to the `4 s` per-enabled-rock or `35 s` corner-matrix gates, and no runtime generation, clipping, bevel, triangulation, normal, shader, serialized, or Inspector owner is altered.

### EW-C1A.3c complete preflight and one-build ownership

The candidate-search boundary is now the last non-emitting point before shell construction. `EdgeWearEvaluationMode.CornerDamageIntegrationPreflight` follows the real post-chip path through micro-topology normalization, ranked transaction, construction provenance, full ordinary plus mandatory candidate viability, artistic/coverage selection, shared mandatory scale, topology-context construction, candidate conservation, and `AuditExplicitChamferCornerSolution`. `ApplyEdgeWearCoverageCornerSolution` marks the exact active identities without emitting bevel faces, polygon triangles, final normals, tangents, or a mesh.

The preflight record owns:

- selected and graph-selected candidate counts;
- candidate-conservation, topology-context, and width-solution validity;
- expected/recorded/isolated-certified/selected/solved mandatory counts;
- predicted ordinary and mandatory stable identities;
- predicted unrelated baseline/retained/lost identities;
- resolved common ring scale and exact failure stage.

All ranked corners are evaluated at that boundary. Only the first complete, zero-collateral preflight may reach `CornerDamageIntegrationPreview`, and it receives one complete build. The former next-lower fallback is removed. The emitted result must match the retained transaction and predicted identity sets; disagreement is a first-class `integration-preflight-mismatch`, not permission to continue brute-force generation.

Performance is part of correctness. Cached baseline duration and observed seed integration duration gate admission to the sole synchronous build. Enabled cases hard-stop at `4 s`, the corner matrix at `35 s`, and the complete suite at `58 s`, with a final evidence reserve. The budgets cannot interrupt a single Unity synchronous generation call, but they prevent knowingly starting work that cannot fit and reject any case that returns over budget. No acceptance rule, clipping geometry, solver, triangulation, final-normal threshold, shader, serialized control, production path, or per-frame owner changes.

### EW-C1A.3b bounded staged search and performance ownership

The C1A.3a outer loop treated every ring scale as a complete mesh-generation configuration and repeated the entire search once for the raw endpoint and once for the integrated endpoint. That architecture is retired. EW-C1A.3b preserves the same acceptance boundary but moves scale selection into a bounded preflight:

```text
ranked corner transaction
-> cap-ring-only lifecycle construction
-> isolated maximum certified width for each of 3 ring edges
-> common schedule scale = floor(minimum certified ratio)
-> one complete post-chip integration
-> at most one next-lower fallback
```

`EdgeWearEvaluationMode.CornerDamageRingPreflight` is editor-only. It commits the certified construction faces and identities but asks `BuildEdgeWearBevelCandidates` to skip every non-mandatory edge. Only the three cap-ring candidates enter structural classification, locality, topology-context creation, and isolated bounded-edge width certification. The mode captures that evidence and exits before artistic context, coverage sorting, coexistence, bevel-shell construction, polygon triangulation, final normals, or mesh emission.

The resolved common scale is the largest member of `{1, 0.75, 0.5625, 0.421875, 0.31640625, 0.25}` not exceeding the smallest isolated certified-width ratio. All three ring edges still receive exactly one common multiplier. A ratio below `0.25`, missing mandatory record, or failed isolated audit rejects the corner before any complete integration build. A global failure that only appears in full construction may consume one next-lower fallback; no six-build outer sweep remains.
...
## EW-C1A.3g prepared-candidate and complete-authority boundary

C1A.3f correctly removed complete shell construction from rejected candidate preparation, but incorrectly treated the solve-only retained identities as authoritative and did not preserve the ordinary baseline mesh through failure routing. C1A.3g separates those contracts:

1. `SolvePlaneCutBevelKernel` remains candidate preparation. It freezes post-cut faces, topology context, lifecycle coverage, retained candidate objects, and resolved plane/rail data. Its ordinary/mandatory identity sets are predictive evidence only.
2. Ranked search may reject any number of prepared candidates using transaction, topology, mandatory-ring, width, and predicted unrelated-retention evidence.
3. The first prepared candidate selected for complete evaluation is passed once to `MaterializePlaneCutBevelSolvedPlan`. That call owns conflict reduction, final retained candidates, clean-shell construction, topology/face/volume/bounds certification, triangulation, coverage finalization, and the final `TriangleSoup`.
4. Final ordinary/mandatory identities and unrelated-retention counts are recomputed from completed coverage. These final sets establish the authoritative plan hash. Divergence from prepared estimates is not itself a failure because preparation is not authoritative.
5. Final preview routing consumes the completed soup and status directly. Exact final-plan versus emitted-status identity/hash comparison remains mandatory and no count-equivalent substitution is accepted.
