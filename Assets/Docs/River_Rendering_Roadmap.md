...
**Implemented:** A river-owned, chunked runtime keeps four interaction features independent: height-aware Static Pressure, static Obstruction Wakes, dynamic Moving Trails, and one-shot Impact Ripples. Generated static geometry registers automatically and may inherit, disable, or customize Static Pressure and Obstruction Wake independently. Pressure height is computed from flow, blockage, local mesh support, and Stage 3 wave headroom; Strength selects a normalized point inside that feasible range, Contact Sharpness controls the one-sided upstream falloff, and Profile Variation redistributes supported height across the cached lateral ridge samples. Profile changes are decoupled from Stage 3 waves and use a deterministic randomized interval, with a default range of 0.75–1.75 seconds, an exposed 0.5–3 second authoring range, and generated-object Custom overrides for the minimum and maximum cadence. Each transition is duration-limited to complete before the next scheduled profile change, while geometry support, contact overlap, endpoint taper, and feasible-height limits remain unchanged. Wakes and trails derive transport direction and speed from river flow. Static geometry preparation is cached and frame-budgeted, while shared pressure, wake, and ripple fields keep shader cost independent of active-source count. Stage 5 controls do not participate in riverbed or corridor generation. Dedicated debug views expose each field.
