...
**Generated static-geometry registration and authorship:** Active procedural geometry registers through a neutral event-driven registry. Solid static sources expose their final generated mesh and announce geometry changes; river runtimes perform cheap bounds rejection, derive and cache waterline footprints only for geometry that actually touches the river, and unregister automatically with chunk or object lifetime. Generated static obstacles therefore require no river emitter component. Generated objects may optionally expose a common river-interaction profile for participation, normalized pressure strength, surface detail, response stiffness, wake length, unsteadiness, and footprint padding; objects without that profile inherit the neutral automatic baseline. Future generated rocks, logs, slabs, bridge pieces, and other solid static forms use the same contracts, while dynamic gameplay sources remain emitter-driven.
...
**Implemented:** A river-owned, logically chunked system separates static obstruction pressure, downstream wake energy, and impact ripples. Generated static geometry registers automatically, receives a cached water-contact footprint, and retains object-level authorship without requiring an emitter. Static pressure is rebuilt only when source or river data changes. It now uses several height-aware mesh slices so the ridge follows vertical, sloped, narrowing, and short objects instead of vertically extruding one waterline contour. Its feasible height range is derived from local object support, flow speed, blockage, Stage 3 wave allowance, and the river style; presets choose a normalized position within that range, while Custom exposes the `0–1` pressure strength directly. The ridge overlaps the object-facing boundary, falls only toward open upstream water, and applies deterministic local height variation only while Stage 3 waves are changing the local water level. Static and moving wakes use a separate lower-resolution downstream-advection field, while one-shot impacts alone use the stability-limited ripple field. The shader combines all layers through fixed shared texture samples, with dedicated pressure, wake-source, and wake-energy debug views. Dynamic standing-pressure and collider-derived dynamic footprints remain pending validation.

**Validated:** Implementation remains pending Unity compilation, performance profiling, and visual approval. The V4 continuously driven obstacle-wave solver is rejected and must not be treated as an accepted Stage 5 result.
