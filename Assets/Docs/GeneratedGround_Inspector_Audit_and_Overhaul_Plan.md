...
The GeneratedGround Inspector and Painted Accent production workstream is accepted. No known correctness, authoring, runtime-generation, build-safety, or generated-asset-lifecycle blocker remains in that scope. This is not a closure of GeneratedGround or the Ground visual roadmap. V3S River-Coupled Ground Response is the active milestone; V4 Contact / Edge Accents is queued after V3S and excludes River sources. V3S-A2A extends the main `GeneratedGround` authoring façade with automatic Bank/Riverbed Surface Layer dropdowns, inline reusable-profile editing, and in-place create/duplicate actions; routine River-coupled authoring must not require Project-window asset navigation. V3S-A2B adds the adjacent `River-Coupled Ground Response — Bank Composition` foldout, and A3A adds independent cover-retreat controls. A3B.1 validates delayed Shore Hydrology Modifier creation and assignment. A3B.2 implements and Unity-validates the Inspector consolidation: modifier selection, inline reusable-profile editing, create/duplicate actions, storage ownership, and spatial application controls are grouped under one `River-Coupled Ground Response — Shore Hydrology` foldout after Bank Composition and Surface-Cover Response. V3S-A4A implements one adjacent `River-Coupled Ground Response — Riverbed Composition` foldout immediately after Bank Composition, containing only `Riverbed Material Strength`; it is disabled while Riverbed Surface Layer is `Inherit Primary Ground`. The exact Inspector contract is source-audited and Unity validation is pending. Shared-style/local-override ownership, delayed asset creation, and existing Surface Layer selectors remain unchanged. A4A adds no debug view, profile editor field, asset workflow, Riverbed hydrology control, or new folder. Legacy hydrological fields remain removed from active Surface Layer authoring, and the obsolete Shore Damp control remains hidden.
