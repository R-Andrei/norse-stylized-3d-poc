...
- 5.8 adds chaotic intermittent persistent material drift: stored `Material Presence` can receive coherent lateral impulses, calm/no-drift intervals, local shear, and subtle resistance while remaining net-downstream; 5.8b recalibrates the activity curve, backtraced center influence, lateral impulse scale, shear, and resistance so `Strength = 1` / `Rhythm = 1` is the normal readable baseline.
