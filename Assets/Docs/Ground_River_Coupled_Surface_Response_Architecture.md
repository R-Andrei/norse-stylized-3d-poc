...
**Architecture accepted. V3S-A0/A1, V3S-A2A, the V3S-A2B material-state proof, and V3S-A2C.4 are accepted. The original V3S-A2C ordinary-Ground distance ownership was rejected. V3S-A2C.1 corrected ownership to the River corridor and its bank-distance behavior is visually validated. V3S-A2C.2 failed as an empty-stream migration. V3S-A2C.3 remains an ordinary-Ground mesh-layout integrity invariant. On 2026-07-16 the user confirmed that the bank material spilling into ordinary Ground is solved after V3S-A2C.4; explicit per-renderer authorization and ordinary-Ground River-data cleanup are therefore the frozen isolation baseline. V3S-A3A — Bank Surface-Cover Retention and Retreat is the active implementation patch.**
...
A profile defines material identity only:
...
It does not own River geometry, spatial reach, waterline thresholds, layer blend strength, runtime weather state, or world placement.
...
**Status:** implementation explicitly authorized on 2026-07-16. This persistent plan is recorded after the mandatory read-only gate and before implementation edits. Implementation and post-change validation are pending.
...
**Status:** planned, not authorized for implementation in A3A.

After A3A is Unity-accepted, add Shore Wetness Strength, Shore Wetness Reach, Immediate-Bank Saturation, Waterline Wetness Strength, and Shore Smoothness Boost. Remove Shore from the legacy generic damp/deposit and pooled-wetness paths before applying one bounded local/global wetness union. Wet tint, darkening, pixel softening, smoothness, restrained specular response, and the Painted Accent ordering decision belong to A3B only.

### A3A acceptance contribution

- grass can retreat while exposed soil or sand replaces it;
- snow can thin or disappear across the selected Bank substrate;
- frost can retreat independently from snow;
- Painted Accent ink can be retained or suppressed without changing its source coverage;
- all retreat remains independent from wetness and ordinary damp/deposit behavior.

## V3S-A4 — Riverbed surface composition

**Purpose:** apply the selected Riverbed Surface Layer through exact `UV3.x` Riverbed Support.
...
Riverbed Wetness
Riverbed Smoothness Boost
```

Resolution order:

```text
ordinary Ground material
→ Bank Surface Layer
→ Riverbed Surface Layer
→ submerged cover exclusion
→ local bank / bed wetness
→ global wetness union
```

The riverbed layer wins where bank and bed signals overlap. Centre, FlatBedEdge, and BedSlope receive the selected substrate; HiddenCover, OuterBlend, and BuriedApron do not. Water optics remain River-owned.

### Acceptance

- zero Riverbed Material Strength preserves inherited Ground;
- complete visible bed support receives the selected layer;
- non-bed corridor regions receive no bed material;
- vegetation and snow are absent from submerged support;
- sandy bank + gravel bed, soil bank + mud bed, and inherited mudflat configurations are possible without code changes.
...
Patch 7 — V3S-A3B
    shore and waterline hydrology
    legacy Shore wetness refactor
    bounded effective wetness

Patch 8 — V3S-A4
    Riverbed Surface Layer composition through UV3.x
    submerged cover exclusion
    permanent bed wetness and finish
...
Unity-validate the source-implemented V3S-A3A patch. Confirm the zero-default compatibility baseline, four independent retreat channels, debug modes `36–37`, ordinary-Ground exclusion, and persistence through material refresh and regeneration. Do not begin A3B hydrology or any River-file change until A3A is visually accepted.
