...
Dry Riverbed material identity now has its own inward transition, independent from Riverbed wetness. `Material Blend Distance` and `Material Blend Softness` use the existing exact Riverbed Support and inward-distance stream. At the support edge, the Riverbed material weight is zero. Moving inward, the complete Riverbed response reaches the authored material strength over the configured distance. The complementary edge response is the resolved Bank material when a Bank layer is active; otherwise it remains Primary Ground. Palette/form, slope/normal, cavity, dry finish, roughness, and texture-form lighting use the same normalized substrate weights. Cover exclusion remains exact-support-owned and does not fade with the dry material.

The historical texture-form `0.5` binary substrate cut remains a compatibility path outside the enabled Riverbed transition and when the new distance is zero. It is disabled inside an enabled dry Riverbed transition so the authored interpolation cannot collapse back to a hard line.

The proof payload advances to algorithm version `6`. Exact placement, mechanical coverage, source identity, and internal rock response remain unchanged. Final mask downsampling now retains fractional 2×2 work-pixel coverage, and a deterministic toroidal seven-tap separable silhouette field softly interpolates only the outer runtime payload from substrate into rock. Internal form and placement remain intact. The proof reports fractional silhouette coverage and maximum adjacent Palette Form step.

The installer remains one explicit action at fixed canonical paths. Reruns compare payload hashes and importer state, reconcile the dedicated three-entry library by stable ID, update existing material/layer references in place, preserve user-authored palette and layer tuning, rebuild the generated array sub-assets inside the existing library, verify canonical GUID preservation, and report `Created`, `Updated`, or `Unchanged`. It never creates numbered copies as part of its normal path. Same-name assets outside its dedicated paths are reported but not modified automatically.

No River producer, geometry stream, wetness formula, scene, prefab, renderer, component, tag, layer, or draw-call architecture changes.

---

## 2026-07-21 — GSU-M2.7C.5E.1.1: Baseline-safe editor integration correction

M2.7C.5E.1.1 preserves the paired `PaletteForm + RuntimePackedDetail` library architecture, three runtime comparison candidates, editable material-profile palettes, generated array ownership, and existing Ground layer selection path. It corrects only the source-delivery boundary.

`GeneratedGroundEditor.cs` is no longer distributed by this feature. The previous package shipped a complete editor file from an archive baseline containing vegetation-coverage integrations that were incompatible with the user's live branch. The corrected architecture treats that large shared custom editor as branch-owned source that may only be changed against the actual live file.

Guaranteed palette authoring for the three comparison surfaces is through the generated `SSMP_RiverbedUltraSparse`, `SSMP_RiverbedVerySparse`, and `SSMP_RiverbedSparse` assets. Inline display of those same palette controls inside the Ground Inspector is a convenience, not a runtime requirement, and is deferred until a branch-aligned editor patch is available.

No runtime sampling, shader, Ground component, vegetation, River, scene, prefab, layer, or tag architecture changes.

---

## 2026-07-21 — GSU-M2.7C.5E.1: Three-candidate runtime surface comparison architecture

> **Source-delivery correction:** M2.7C.5E.1.1 preserves this runtime architecture but excludes the branch-owned `GeneratedGroundEditor.cs` replacement.

M2.7C.5E.1 promotes all three colour-neutral sparse-riverbed payload candidates into reusable selectable Ground surfaces for real game-camera comparison. No density winner is selected by this patch.

The active runtime-comparison assets are:

- Riverbed — Ultra Sparse: six generated rock placements;
- Riverbed — Very Sparse: nine generated rock placements;
- Riverbed — Sparse: twelve generated rock placements.

Each candidate uses one dedicated Palette Form plus Runtime Packed Detail pair generated by M2.7C.5D.5. Palette preview variants are not separate surface identities. Every candidate owns one editable `StylizedSurfaceMaterialProfile`, and Base, Dark, Light, Cavity, Texture Form Strength, Scene Lighting Response, world scale, normal, cavity, roughness, and dry-finish values remain ordinary material controls.

The shared surface-detail library now supports a third additive source mode: `PrepackedDetailWithTextureForm`. It copies one linear packed-detail source and one pre-normalized sRGB Palette Form source directly into the existing generated arrays. It does not run the authored-material normalization path, so the accepted substrate/rock form hierarchy is preserved. Existing prepacked-only and five-map authored-material entries retain their current behavior and serialized values.

The shader and runtime sampling contract are unchanged. The existing packed array supplies slope, cavity, and roughness; the existing texture-form array supplies the normalized Dark/Base/Light form; existing material properties supply the editable palette and response strengths. This patch adds array entries and selectable profiles only, not another texture sample, shader branch, renderer, draw call, component, or per-frame process.

Installation is an explicit idempotent Editor action:

```text
Tools > PS3D > Install All Sparse Riverbed Surface Candidates
```

It consumes a passing local M2.7C.5D.5 proof, copies six source payloads into an editor-only ArtSources folder, creates or verifies one dedicated three-entry library, creates three material profiles and three Ground layer profiles, rebuilds the generated arrays, verifies both slice mappings, and writes/copies one report. Reruns update source payload bytes and ownership references but preserve existing material and Ground-layer tuning.

The installer does not assign any scene object. Comparison remains author-owned through the existing Custom Riverbed Surface Layer selector. A later freeze step may retire unselected candidates after production-camera evaluation.

The comparison library is deliberately temporary and stores all three `1024×1024` packed/form slice pairs together. In RGBA32 with complete mip chains, the analytical texture payload is approximately `32 MiB` before Unity/driver overhead. This is a comparison-only performance exception; after one density is selected, the production library should be compacted to one slice pair, approximately `10.7 MiB` including mips. Runtime sampling and draw-call count remain unchanged.

---

## 2026-07-21 — GSU-M2.7C.5D.5: Palette-neutral sparse riverbed payload architecture

M2.7C.5D.5 preserves the accepted M2.7C.5D.4 substrate structure and the M2.7C.5D.3 exact-count nested `6 / 9 / 12` sparse-rock architecture. Frozen Generated Mass sources, processed rock response, unified wear `0.52`, fallback wear `0.56`, placement, hotspot, scale, root, seam, and substrate contracts remain unchanged.

The new architectural boundary separates **material form** from **material colour**:

- `PaletteForm` is a normalized grayscale payload. `0.5` is the Base pivot, values below `0.5` select toward Dark, and values above `0.5` select toward Light. The PNG stores gamma-encoded bytes so an sRGB runtime form texture decodes to the intended linear form values.
- The homogeneous substrate occupies a restrained Base-to-Light band while preserving its accepted micro-noise structure.
- Rocks occupy predominantly the Dark-to-Base band. Their form is deterministically extracted from the accepted frozen Moderate scalar response, preserving plane, root, and wear hierarchy without retaining RGB colour authority.
- `RuntimePackedDetail` uses the existing runtime detail contract: `R/G` encode signed world-X/world-Z slope, `B` encodes root/contact cavity, and `A` encodes bounded roughness.

Base, Dark, Light, and Cavity colours are runtime material/profile authority. Increasing visual contrast is a palette change, not a substrate-noise regeneration. The proof therefore renders Neutral, Higher Contrast, and Alternate previews from the same immutable `PaletteForm` and `RuntimePackedDetail`; palette previews may differ, but geometry, placement, substrate structure, masks, normals, cavity, roughness, and paired-payload fingerprint may not.

M2.7C.5D.5 remains Editor-only evidence. It does not import assets or modify the runtime detail library, shader, Ground profile, scene, prefab, River, or Inspector. The current library does not yet expose a direct prepacked-detail-plus-pre-normalized-form source mode; selected-candidate promotion and that paired-payload integration belong to separately approved M2.7C.5E work.

---

## 2026-07-21 — GSU-M2.7C.5D.4: Micro-noise homogeneous substrate architecture

M2.7C.5D.4 preserves the M2.7C.5D.3 exact-count sparse rock architecture: the frozen 18-rock Generated Mass library, algorithm-8 source contract, unified wear `0.52`, fallback wear `0.56`, processed rock channels, unique-source nested `6 / 9 / 12` placements, spacing/hotspot limits, mostly-small scales, root sectors, accepted Moderate rock response, and Editor-only evidence ownership all remain unchanged.

The only architectural change is the substrate. One fixed seamless shared muddy field still serves all candidates, but its visual grammar is now micro-noise-first and macro-homogeneous. Broad authored patches are prohibited. The field is built from restrained periodic value-noise bands at approximately `18`, `37`, `79`, `151`, and `281` lattice periods across the tile, with low-amplitude warm greige muddy drift and no named pale/damp lobes.

M2.7C.5D.4 also promotes macro-homogeneity to an explicit architectural validation requirement. Seamlessness and global contrast are no longer sufficient by themselves; the substrate must additionally keep large-window block means close to the global mean so 3×3 repetition and zoom-out do not reveal broad light/dark islands. `SubstrateOnly` and `SubstrateOnly_3x3` remain the primary visual gate, and the validator now records and enforces block-mean deviation metrics at multiple window scales.

M2.7C.5D.4 remains evidence-only. It creates no runtime Ground material, shader, texture array, profile, scene, prefab, River integration, or Inspector control. Runtime integration remains blocked until one complete candidate is visually accepted.

---

## 2026-07-21 — GSU-M2.7C.5D.3: Noise-first ultra-sparse riverbed architecture

M2.7C.5D.3 preserves the frozen 18-rock Generated Mass library, algorithm-8 source contract, unified wear `0.52`, fallback wear `0.56`, processed rock channels, root sectors, and accepted Moderate rock response. It changes only Editor-only assembly composition and evidence.

The shared muddy substrate is now the primary material. One fixed seamless periodic value-noise field is generated once per suite and reused byte-for-byte by all three candidates. Its broad, medium, and fine components remain low contrast; it contains no named pale/damp lobes, intentional composition blobs, or candidate-specific substrate seed.

Rock composition is an exact-count nested sequence:

- Ultra Sparse: 6 placements;
- Very Sparse: the same first 6 plus 3 placements;
- Sparse: the same first 9 plus 3 placements.

Every placement uses a different frozen rock source. Centres are sampled uniformly in the toroidal domain and accepted through radius-aware spacing, overlap, milestone coverage guardrails, and explicit near/broad hotspot limits. Positive pocket fields, quiet-block placement gating, source-count equalization, forced accent rocks, and presentation-origin reframing are removed. The 32×32 quiet-block fraction remains descriptive evidence only.

Scale evidence is mostly small: a deterministic finite realization uses 75% small rocks in `0.55–0.80` and 25% medium rocks in `0.80–1.05`; the permitted range remains `0.55–1.20`, but no large placement is forced. Visual acceptance depends on `SubstrateOnly`, `SubstrateOnly_3x3`, candidate Moderate/3×3, placement debug, channels, and mips rather than numerical promotion.

M2.7C.5D.3 remains evidence-only. It creates no runtime Ground material, shader, texture array, profile, scene, prefab, River integration, or Inspector control. Runtime integration remains blocked until one complete candidate is visually accepted.

The active handmade 2D sparse-riverbed candidate scripts are deleted as reconciliation with the already accepted retirement architecture. Historical handmade sections below remain superseded context only.

---

## 2026-07-21 — GSU-M2.7C.5D.2: Sparse light-mud assembly architecture

M2.7C.5D.2 preserves the frozen 18-rock Generated Mass library, algorithm-8 source snapshot, unified wear `0.52`, fallback wear `0.56`, and accepted Moderate rock response. It changes only Editor-only tile composition and substrate evidence.

Authoritative assembly direction:

- active candidates are Very Quiet `4.5%`, Quiet `6.0%`, and Natural `7.5%`; the M2.7C.5D.1 Dense candidate is retired;
- rock scale uses a weighted `0.55–1.55` distribution with many small/medium stones, fewer large stones, and rare large accents;
- source diversity remains at least 12 IDs with maximum 12% share, but exact equal-use balancing is not desired;
- composition uses irregular positive pockets, explicit broad quiet regions, occasional isolated rocks, and pre-commit quiet-block enforcement;
- the toroidal field receives a deterministic quiet-block-aligned presentation offset chosen to reduce edge crossings, side imbalance, and off-centre framing without changing periodicity;
- the substrate is lighter mud with restrained periodic pale-silt and darker-damp patches/streaks, rather than a uniform dark-brown fill;
- assembly-only root limiting retains the strongest broken contact sectors and caps affected perimeter at 62%; the isolated-rock root architecture remains frozen.

M2.7C.5D.2 remains evidence-only. No runtime Ground material, shader, texture array, profile, scene, prefab, River integration, or Inspector control is created. One complete candidate must pass visual review before M2.7C.5E runtime integration.
...
V3M Broad Macro Patch Completion and V3R Ground Elevation Readability are accepted. V3S River-coupled placement, composition, cover response, hydrology, submerged finish, waterline highlight, and inward Riverbed transition are Unity-validated and frozen through A4B.3. **GSU-M1 Reusable Stylized Surface Material Foundation** is implemented and source-audited through **GSU-M1.9A.5 — Source-Art Packed Conversion, Macro Rebalance, and Worn Edge Accent**. GSU-M1.3.1 guards transient missing-array transport; GSU-M1.7 adds inline shared-material editing and neutral Bank/Riverbed application multipliers; GSU-M1.7.1 corrects the Unity 6.5 `EntityId` compile blocker; GSU-M1.8 retains the accepted 256² runtime tier. Fine Gravel payloads M1.6 through M1.9A.4 are visually superseded. M1.9A.5 is the active two-candidate texture gate and changes no C#, HLSL, ShaderLab, River source, renderer, geometry, UV3, mask, hydrology, scene, prefab, or canonical Fine Gravel assignment. Generic Pixel Surface profiles still own dry material identity; Ground Surface Layers add only Ground cover compatibility; River banks and beds remain consumers. Unity array rebuild, gameplay visual acceptance, and profiling remain pending. Family tuning follows material acceptance; V4 Contact / Edge Accents remains queued afterward and excludes River sources.
...
GSU-M1.8  — 256 runtime restoration; texture visually superseded
GSU-M1.9A.5 — source-art worn-edge Fine Gravel A/B payload; Unity acceptance pending
...
#### Fine Gravel visual contract — GSU-M1.9A.5
