...
The current Chipping baseline, Stage 3 shore-wave controls, `4.11C.5.18C`, `4.11C.5.18D`, `4.11C.5.18E`, and `4.11C.5.18F.1` are Unity-validated and accepted. `5.18E` fully fixed the short visible-lifetime problem, and `5.18F.1` made Arc/Semi-Arc cohorts detach and travel correctly. `4.11C.5.18H.4 — Event-Owned Analytic Open-C Geometry` is Unity-validated as the accepted no-wrap baseline. `4.11C.5.18H.6.2 — Thin Mesh Profile and Front-Persistent Semi-Arc Lifecycle` is the active correction; `5.18H.6` is the Unity-observed mesh-fit baseline and `5.18H.6.1` is rejected; `5.18H.5` remains only the Unity-observed signed-offset intermediate. It retains the `5.18G` per-anchor Build/Hold/Progressive Release/Rest scheduler, supersedes the oversized `5.18G.1` near-ring mantle, and replaces the bounds-derived `5.18H.4/5.18H.5` connector with one cached mesh-fitted upstream profile plus straight downstream arms. The old Stage 8 reflection/final-integration plan is stale and retired. All performance work remains deferred to one future comprehensive River performance pass.
...
Current active direction: import and validate `4.11C.5.18H.6.2`. A C# compile and compute reimport are expected. In `Automatic Birth Sources`, Arc must follow both halves of the prepared mesh-fitted waterline profile and retain two straight downstream arms. Semi-Arc must follow exactly one profile half from its selected physical shoulder to the physical upstream point and contain exactly one downstream arm. Pattern/mix changes must retire old Arc/Semi emitters without clearing already deposited material. Along-Flow and Across-River Contact Offsets remain signed and support-agnostic, with zero using the physical profile. Contact Fleck, Shore, Free Water, velocity, transport, lifecycle, support, duty-cycle timing, and RiverCorridor/Ground contracts must remain unchanged. The full River performance pass remains deferred; steady-state resources and dispatch count must remain unchanged.
...
`5.18H.6.1` is rejected because it broadened the mesh-profile feather from `0.38` to `0.55` cell and replaced the accepted direct five-point waterline profile with an expanded exterior support envelope. The runtime result was visibly double-thick and lower fidelity. `5.18H.6.2` restores the exact H.6 profile producer and exact H.6 raster coefficients.

Semi-Arc Build now begins at the physical front point, progresses across only the selected front half, reaches its shoulder, and then extends to the one downstream arm tip. Reverse Release removes the arm first and preserves the front until the final interval. Full Arc progression remains terminal-to-terminal. Actual within-half segment lengths reuse existing event lanes; Arc/Semi source fill remains forced solid so those lanes cannot create holes.

Performance/resource contract remains zero-new-resource and zero-new-dispatch.
...
## Thin Open-C Object Ribbon Arcs — `4.11C.5.18H` / `5.18H.2` — thin width accepted; rear-following geometry superseded by `5.18H.3`
...
`5.16E.2` makes normal Final Foam consume the current committed Layer C state directly and removes `_FoamRenderAdvectionSeconds`, render-side velocity reconstruction, obstacle prediction fading, and residual backtracing. Both Final Foam visibility policies remain. The `0.05` Supported Aging Rate minimum and final-substep lifecycle aging remain.

`Foam Committed Final Preview` is removed as redundant. Debug value `16` safely falls back to Final. `Foam Evaluated Final Preview` remains diagnostic-only; Layer D is not promoted to production by this patch.

No new resource is added. The two Layer C packed textures remain necessary simulation ping-pong storage. The grey interior/white border is a separate Layer E styling issue. `5.16E.2` is Unity-validated and accepted. `5.16E.3` has since attributed the remaining sub-1% transport capacity loss, and `5.16E.3C` records its conscious PoC deferral without changing the solver.

Canonical velocity files remain unchanged.

## Accepted River Inspector and diagnostics contract — R1–R6

The River Inspector redesign is complete and Unity-validated through R5. R6 records the result canonically and removes the temporary implementation plan.
...
production authoring grouped by feature;
Foam authoring and diagnostics grouped by Layers A–E;
one exclusive Debug Views hub;
read-only stable-height Runtime Diagnostics;
read-only Generated Status;
all mutating/test tools under Actions;
constant repaint only for visible live diagnostic leaves.
```

The debug hub retains the existing serialized enum fields. Explicit selection is exclusive. Legacy conflicting selections are reported without silent changes and follow shader priority `Foam > Disturbances > Refraction > Surface Motion > Water Body`; use `Normalize to Rendered View` or `Reset All Debug Views` deliberately.

The capacity attribution continuation is complete and accepted:

```text
4.11C.5.16E.3 — Transport Presence Capacity-Loss Attribution Audit
Runtime Diagnostics > Foam > Layer C — Material & Lifecycle > Transport Accounting
```

Controlled automatic-source results:
...
Obstacle-capacity, state-validity, minimum-cutoff, and attribution-residual values were zero in every capture. Category hit counts may overlap; `Total` is their union. The metric buffer remains 23 raw uints / 92 bytes and the audit adds no texture, field, channel, kernel, dispatch, or readback.

`4.11C.5.16E.3C` retains `0.10%` as the engineering target but temporarily tolerates measured results below a `1.00%` review threshold for the PoC. This is a recorded limitation, not a numerical correction. Exact multi-pass transport and the cheaper estimated `+10–25%` single-pass cap are deferred. Fractional shoreline storage relaxation remains a later zero-dispatch A/B candidate only.

The current continuation contract is now locked by:
...
`5.18C` is Unity-validated and accepted. It retains `5.18B`'s shared automatic production/debug source evaluator but removes cumulative diagnostic history and corrects the thick source/contact geometry that the debug view exposed.
...
The user completed the validation gate and confirmed that the patch works as intended. The `0.50`-texel pressure floor and current reach defaults are accepted for this milestone.
## Final Stage 7 closure — `4.11C.5.18D` Object Birth Control Semantics

`5.18D` preserves all `5.18C` source geometry and serialized numeric values. Historical note: `5.18D` presented the existing Arc/Semi-Arc `Width` fields as `Profile Scale`, because they own reveal/profile behavior inside the fixed shell, and presents Fleck `Width` as `Fleck Size`, because it owns capsule geometry. Contact-normal thickness remains structural one-cell adjacency.

The existing Automatic Birth Sources view reports only two additional compact evidence rows: current one-cell metre dimensions and raw unpadded object half-extent ranges. No new view or runtime resource is added. Source-contract comments now document the accepted ownership split; compute/source arithmetic remains untouched.

After normal Unity import confirmation, there is no active River visual patch. Do not revive Stage 8 reflections, Fray, Remaining-Life morphology, or isolated optimization work from historical sections. Any comprehensive River performance pass requires its own explicit scope.
