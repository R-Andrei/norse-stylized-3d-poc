...
namespace ProgrammaticStylized3D.Rivers
{
    ...
    public sealed class StylizedRiver : MonoBehaviour
    {
        ...
        [Tooltip("Selects where inherited generated objects sit between the computed minimum and maximum feasible pressure heights.")]
        [Range(0f, 1f)]
        [SerializeField] private float staticPressureStrength = 0.65f;

        [Tooltip("Controls how abruptly the pressure ridge descends away from an obstruction. Higher values produce a steeper contact face without increasing the computed height ceiling.")]
        [Range(0.5f, 4f)]
        [SerializeField] private float staticPressureContactSharpness = 2.8f;

        [Tooltip("Controls how strongly inherited static-pressure ridges redistribute their supported height laterally. Zero keeps the cached geometry-derived profile stable.")]
        [Range(0f, 2f)]
        ...
        [Tooltip("Controls downstream wake energy generated by inherited static obstructions.")]
        [Range(0f, 3f)]
        [SerializeField] private float obstructionWakeStrength = 1.50f;

        [Tooltip("Controls how far inherited obstruction wakes remain active downstream.")]
        [Range(0.25f, 3f)]
        ...
        [Range(0f, 3f)]
        [SerializeField] private float movingTrailStrength = 1.35f;

        [Tooltip("Controls how long moving trails remain visible after the emitter passes.")]
        [Range(0f, 1f)]
        [SerializeField] private float movingTrailPersistence = 0.65f;

        [Tooltip("Controls the lateral width of moving-emitter trails.")]
        [Range(0.5f, 2f)]
        [SerializeField] private float movingTrailWidth = 1f;
        ...
        public float DisturbanceStrength => movingTrailStrength;
        ...
        private void ValidateSettings()
        {
            ...
            movingTrailStrength = Mathf.Clamp(
                ...
                0f,
                3f);
            movingTrailPersistence = Mathf.Clamp01(
                movingTrailPersistence);
            movingTrailWidth = Mathf.Clamp(
                movingTrailWidth,
                0.5f,
                2f);
